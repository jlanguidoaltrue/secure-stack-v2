...
export const updateMe = async (req, res, next) => {
  try{
    ...
    const u = await User.findByIdAndUpdate(req.user.sub, update, { new: true }).select("-passwordHash").lean();
    res.json({ data: u });
  }catch(e){ next(e); }
};
...
export const updatePhoto = async (req, res, next) => {
  try{
    ...
    const u = await User.findByIdAndUpdate(req.user.sub, { avatarUrl: url }, { new: true }).select("-passwordHash").lean();
